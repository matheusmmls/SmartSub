...
public class SmartSubConsole {
    private GerenciadorFinanceiro gerenciador;
    private GerenciadorArquivo arquivo;
    private Scanner scanner;

    public SmartSubConsole() {
        this.gerenciador = new GerenciadorFinanceiro();
        this.arquivo = new GerenciadorArquivo();
        this.scanner = new Scanner(System.in);
        this.gerenciador.setAssinaturas(this.arquivo.carregar());
    }
    ...
    private int lerSubMenuValido(Scanner scanner) {
        while (true) {
            try {
    ...
    private double lerValorValido(Scanner scanner) {
        while (true) {
    ...
    private int lerDiaValido(Scanner scanner) {
        while (true) {
            String entrada = scanner.nextLine().trim();
            if (entrada.equalsIgnoreCase("sair")) {
                return -1;
    ...
    private int lerIndexValido(Scanner scanner, int tamanhoLista) {
        while (true) {
            String entrada = scanner.nextLine().trim();
            if (entrada.equalsIgnoreCase("sair")) {
                return -2;
            ...
            try {
                ...
                System.out.print("Número fora da lista! Digite um número válido correspondente (ou 'sair' para cancelar): ");
            } catch (NumberFormatException e) {
